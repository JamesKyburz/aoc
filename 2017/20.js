function parse (input) {
  return input
    ...
    .map((line, i) => {
      ...
      return {
        p: numbers.splice(0, 3),
        v: numbers.splice(0, 3),
        a: numbers.splice(0, 3),
        i
      }
    })
}

function compute ({ p, v, a }) {
  v[0] += a[0]
  v[1] += a[1]
  v[2] += a[2]
  p[0] += v[0]
  p[1] += v[1]
  p[2] += v[2]
  return p.map(Math.abs).reduce((a, b) => a + b)
}
...
function part1 () {
  ...
  const i = particles.sort((a, b) => compute(a) - compute(b))[0].i
...
function part2 () {
  ...
  console.log('initial particle count', particles.length)

  while (--times) {
    const positions = {}
    for (const particle of particles) {
      compute(particle)
      const key = [...particle.p]
      positions[key] = positions[key] || 0
      positions[key]++
    }
    for (const key of Object.keys(positions)) {
      if (positions[key] > 1) {
        particles = particles.filter(x => x.p.toString() !== key)
      }
    }
  }
