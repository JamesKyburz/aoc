for (const { passphrase, valid, anagramCheck } of [
  { passphrase: 'aa bb cc dd ee', valid: true },
  { passphrase: 'aa bb cc dd aa', valid: false },
  { passphrase: 'aa bb cc dd aaa', valid: true },
  { passphrase: passphrases(), valid: 383, anagramCheck: false },
  { passphrase: 'abcde fghij', valid: true, anagramCheck: true },
  { passphrase: 'abcde xyz ecdab', valid: false, anagramCheck: true },
  { passphrase: 'a ab abc abd abf abj', valid: true, anagramCheck: true },
  { passphrase: 'iiii oiii ooii oooi oooo', valid: true, anagramCheck: true },
  { passphrase: 'oiii ioii iioi iiio', valid: false, anagramCheck: true },
  { passphrase: passphrases(), valid: 265, anagramCheck: true }
]) {
  if (Array.isArray(passphrase)) {
    if (anagramCheck) {
      const expected = passphrase.filter(x => check(x) && !anagram(x)).length
      if (expected !== valid) {
        throw new Error(`incorrect passphrase count ${expected} !== ${valid}`)
      }
    } else {
      const expected = passphrase.filter(check).length
      if (expected !== valid) {
        throw new Error(`incorrect passphrase count ${expected} !== ${valid}`)
      }
    }
    ...
    if (anagramCheck) {
      const expected = check(passphrase) && !anagram(passphrase)
      if (expected !== valid) {
        throw new Error(`${passphrase} ${expected} !== ${valid}`)
      }
    } else {
      const expected = check(passphrase)
      if (expected !== valid) {
        throw new Error(`${passphrase} ${expected} !== ${valid}`)
      }
    }
...
function anagram (passphrase) {
  const sort = x => [...x].sort().join('')
  const words = passphrase.split(' ')
  for (const word of words) {
    if (words.filter(x => sort(x) === sort(word)).length !== 1) return true
  }
}
