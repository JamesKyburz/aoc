...
for (const { input, expected, solution } of [
  { input: example, expected: 1, solution: max },
  { input: example, expected: 10, solution: highest },
  { input: puzzleInput(), expected: 3612, solution: max },
  { input: puzzleInput(), expected: 3818, solution: highest }
]) {
...
function highest (input) {
  return max(input, true)
}

function max (input, highest) {
  const f = {
    inc: (a, b) => a + b,
    dec: (a, b) => a - b,
    '==': (a, b) => a === b,
  ...
  for (const line of lines) {
    const [a, i, n, _, b, o, c] = line.split(/\s/g)
    for (const x of [a, b]) r[x] = r[x] || 0
    if (f[o](r[b], +c)) {
      r[a] = f[i](r[a], +n)
      h = Math.max(h, r[a])
    }
  ...
  if (highest) return h
  return r[
    Object.keys(r).sort((a, b) => r[b] - r[a])[0]
  ]
}
