...
`
for (const { rows, expected, solution } of [
  { rows: example1, expected: 18, solution: part1 },
  { rows: input, expected: 32020, solution: part1 },
  { rows: example2, expected: 9, solution: part2 },
  { rows: input, expected: 236, solution: part2 }
]) {
  const actual = solution(rows)
  if (actual !== expected) { throw new Error(`rows: ${rows} actual: ${actual} !== expected: ${expected}`) }
}
...
function part2 (rows) {
  return rows
    ...
    .map(row => {
      for (const a of row) {
        for (const b of row.filter(x => x > a)) {
          if (b % a === 0) return [b, a]
        }
      }
    })
